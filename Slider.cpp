//-----------------------------------------------------------------
// Slider Object
// C++ Source - Slider.cpp - version 2008 v2_06_3
// Copyright Peter Ver- kevin.hoefman@howest.be
...
Slider::Slider(ResourceLoader* resourceLoaderPtr, String const& textRef) : m_x(0), m_y(0), m_BgColor(RGB(255, 255, 255)), m_ForeColor(RGB(0, 0, 0)), m_BgColorBrush(0), m_Font(0), m_OldFont(0)
{
	// Create the edit box
	m_hWndEdit = CreateWindow(TEXT("EDIT"), textRef.ToTChar(), WS_BORDER | WS_CHILD | WS_CLIPSIBLINGS | WS_TABSTOP | ES_LEFT | ES_AUTOHSCROLL, 0, 0, 0, 0, resourceLoaderPtr->GetWindowHandle(), NULL, GetModuleHandle(0), NULL);
...
Slider::Slider(ResourceLoader* resourceLoaderPtr) : m_x(0), m_y(0), m_BgColor(RGB(255, 255, 255)), m_ForeColor(RGB(0, 0, 0)), m_BgColorBrush(0), m_Font(0), m_OldFont(0)
{
	// Create the edit box
	m_hWndEdit = CreateWindow(TEXT("EDIT"), TEXT(""), WS_BORDER | WS_CHILD | WS_CLIPSIBLINGS | WS_TABSTOP | ES_LEFT | ES_AUTOHSCROLL, 0, 0, 0, 0, resourceLoaderPtr->GetWindowHandle(), NULL, GetModuleHandle(0), NULL);
...
void Slider::SetBounds(int x, int y, int width, int height)
{
	m_x = x;
	m_y = y;
...
void Slider::SetText(String const& textRef)
{
	SendMessage(m_hWndEdit, WM_SETTEXT, 0, (LPARAM) textRef.ToTChar());
}

void Slider::SetFont(String const& fontNameRef, bool bold, bool italic, bool underline, int size)
{
	LOGFONT ft;
...
void Slider::SetForecolor( COLORREF color )
{
	m_ForeColor = color;
...
void Slider::SetBackcolor( COLORREF color )
{
	m_BgColor = color;
...
COLORREF Slider::GetForecolor()
{
	return m_ForeColor;
}

COLORREF Slider::GetBackcolor()
{
	return m_BgColor;
}

HBRUSH Slider::GetBackcolorBrush()
{
	return m_BgColorBrush;
}

LRESULT Slider::EditProcStatic(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	#pragma warning(disable: 4312)
	return reinterpret_cast<Slider*>(GetWindowLong(hWnd, GWL_USERDATA))->EditProc(hWnd, msg, wParam, lParam);
	#pragma warning(default: 4312)
}

LRESULT Slider::EditProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{		
